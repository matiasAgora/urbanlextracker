...
# -*- coding: utf-8 -*-
"""
Urban Lex Tracker - Digest de Email v1.2
Siempre envia email: con novedades o con mensaje de "sin novedades hoy".
Deduplica usuarios por email para evitar envios dobles.
"""
...
SOURCE_LABELS = {
    ...
    "contraloria":    "Contraloria General",
}
...
def generate_summary(publications: list[dict], claude) -> str:
    ...
    prompt = (
        ...
        f"Resume en 5-8 vinetas concisas y practicas para arquitectos y revisores "
        f"estas {len(publications)} novedades normativas. "
        f"Indica el impacto practico de cada una:\n{items}"
    )
    try:
        ...
        return "Resumen no disponible en este envio."
  ...
  <div style="border:1px solid #d5e8f0;border-top:none;padding:24px;border-radius:0 0 8px 8px">
    <div style="text-align:center;padding:32px 0">
      ...
      <p style="color:#555;font-size:15px;line-height:1.6;max-width:420px;margin:0 auto">
        Al momento del envio de este digest no se detectaron publicaciones
    ...
    <div style="background:#f4f9fd;padding:16px 20px;border-radius:6px;margin-top:8px">
      <p style="margin:0;font-size:13px;color:#555;line-height:1.7">
        ...
        Diario Oficial · MINVU/DDU · Contraloria General · Biblioteca del Congreso · Proyectos de Ley
      </p>
    </div>
    <p style="font-size:13px;color:#888;margin:24px 0 0;line-height:1.6">
      ...
      recibiras el digest completo con resumen ejecutivo y links directos.
    </p>
    <hr style="border:none;border-top:1px solid #eee;margin:28px 0">
    <p style="font-size:11px;color:#aaa;margin:0">
      ULT - Urban Lex Tracker &nbsp;·&nbsp; By AGORA Revisores<br>
      Recibes este digest porque estas suscrito al seguimiento normativo urbanistico.
    </p>
  </div>
</body>
</html>"""
...
def build_html(publications: list[dict], summary_md: str, fecha: str) -> str:
    ...
    for slug, pubs in by_source.items():
        ...
        items_html += f'<h3 style="color:#1a5276;margin-top:24px;font-size:15px">{label}</h3>'
...
<body style="font-family:Arial,sans-serif;max-width:640px;margin:0 auto;padding:20px;color:#222">
  ...
  <div style="border:1px solid #d5e8f0;border-top:none;padding:24px;border-radius:0 0 8px 8px">
    <p style="font-size:16px;margin-top:0">
      Tienes <strong>{noun}</strong> desde el ultimo digest.
    </p>
    {items_html}
    <hr style="border:none;border-top:1px solid #eee;margin:28px 0">
...
def log_alerts(db: Client, user_id: str, publications: list[dict]):
    ...
    rows = [
        {"user_id": user_id, "publication_id": p["id"], "channel": "email", "sent_at": now}
        for p in publications
    ]
    if rows:
        try:
...
def main():
    ...
    # 2. Obtener usuarios y DEDUPLICAR por email
    users_raw = get_active_users(db)
    seen_emails: set = set()
    users = []
    for u in users_raw:
        em = u.get("email", "")
        if em and em not in seen_emails:
            seen_emails.add(em)
            users.append(u)
    logger.info(f"Usuarios unicos a notificar: {[u.get('email') for u in users]}")
    ...
    # 3. Marcar publicaciones como procesadas
    if hay_novedades:
